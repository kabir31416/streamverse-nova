...
import { Search, User, Tv, Heart, Home, Newspaper, Film, Baby, Trophy, Radio } from "lucide-react";
      ...
      <AnimatePresence>
        {!isWatching && (
          ...
          >
            <div className="mx-auto flex h-16 max-w-[1600px] items-center gap-2 px-4 sm:px-6 lg:px-8">
                  ...
                  return (
                    <Link
                      ...
                      className={`relative rounded-lg px-3 py-2 text-sm transition-all ${
                        active
                          ? "bg-[#1F1F1F] font-semibold text-foreground shadow-[inset_2px_0_0_0_#E5E5E5]"
              ...
              <div className="ml-auto flex items-center gap-2">
                <button
                  ...
                  className="grid h-10 w-10 place-items-center rounded-full bg-secondary/60 text-muted-foreground transition-all hover:bg-secondary hover:text-foreground"
                  aria-label="Search"
                >
            ...
            {/* Mobile nav */}
            <nav className="scrollbar-hide flex gap-1.5 overflow-x-auto border-t border-border/40 px-4 py-2 lg:hidden">
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    ...
                    className={`flex shrink-0 items-center gap-1.5 rounded-full px-3 py-1.5 text-xs transition-all ${
                      active
                        ? "bg-[#1F1F1F] font-semibold text-foreground ring-1 ring-white/20"
